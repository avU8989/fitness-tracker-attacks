...
class HeartRateService(Service):
    ...
    # ENCRYPT_AUTHENTICATED_READ/WRITE means that the characteristic requires both encryption and authentication
    # the client needs to be paired (link encrypted) and that link must have been established with proper authentication (MIITM protection)
    @characteristic(
        uuid=HEARTRATE_MEASUREMENT,
        flags=CF.NOTIFY | CF.AUTHENTICATED_SIGNED_WRITES | CF.ENCRYPT_AUTHENTICATED_READ | CF.ENCRYPT_AUTHENTICATED_WRITE
    )
